...
The neural networks consists of 4 layers. 2 of them with 128 neurons, 1 of 8 neurons
and 1 of output number of neurons (softmax will be calculated from there).

More layers in the model gives us more complexity, so we have more non-linearity
As the numbers of neurons in each of the layers increases, the loss decreases.
...
Mean accuracy over 10000 images: 51 %
...
class NeuralNetwork(torch.nn.Module):
    ...
    def __init__(self, n_input_feature, n_output):
        ...
        self.fully_connected1 = torch.nn.Linear(n_input_feature, 128)
        self.fully_connected2 = torch.nn.Linear(128, 128)
        self.fully_connected3 = torch.nn.Linear(128, 8)
...
def evaluate(net, dataloader, classes):
    ...
    subplot_titles = [classes[i] for i in labels]

    # TODO: Plot images with class names
    plot_images(images, len(labels)//4, 4, "Last Predictions", subplot_titles)
