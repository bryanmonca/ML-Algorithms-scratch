...
def score_mean_squared_error(model, x, y):
    ...
    Args:
        model : object
            trained model, assumes predict function returns N x 1 predictions
        x : numpy
...
if __name__ == '__main__':
    ...
    # x_values refer to degree of polynomial [1, 2, 3]
    ...
    alphas = [0.0, 1.0, 100.0, 1000.0, 10000.0]
    ...
    for alpha in alphas:
        ...
        # alpha is the lambda (degree of regularization) we talked about in lecture
    ...
    # TODO: Set x values (alphas in log scale )and y values (mse)
    ...
    for alpha in alphas:
        ...
        # TODO: Initialize scikit-learn ridge regression model
        model_ridge_poly = RidgeRegression(alpha=alpha)
        ...
        # TODO: Test model on training set
        score_mse_ridge_poly_train = score_mean_squared_error(model_ridge_poly, x_poly_train, y_train)
        print('Training set mean squared error: {:.4f}'.format(score_mse_ridge_poly_train))

        score_r2_ridge_poly_train = model_ridge_poly.score(x_poly_train, y_train)
        ...
        # TODO: Test model on validation set
        score_mse_ridge_poly_val = score_mean_squared_error(model_ridge_poly, x_poly_val, y_val)
        print('Validation set mean squared error: {:.4f}'.format(score_mse_ridge_poly_val))

        score_r2_ridge_poly_val = model_ridge_poly.score(x_poly_val, y_val)
        ...
        # TODO: Test model on testing set
        score_mse_ridge_poly_test = score_mean_squared_error(model_ridge_poly, x_poly_test, y_test)
        print('Testing set mean squared error: {:.4f}'.format(score_mse_ridge_poly_test))

        score_r2_ridge_poly_test = model_ridge_poly.score(x_poly_test, y_test)
