...
In this assignment, we implemented our version of the Ridge Regression Closed form solution and we compared the scores
(mse and r2), using different lambada values, with the scores of scikit-learn's implementation. The dataset we used is the 
Boston Housing Prices with a polynomial expansion of degree 2. The results obtained in both implementations were
very similar, as we can see in the graphs and in the results section.

The loss we minimized was a constrained version of minimum squared error loss. It is composed by a data fidelity loss
(mse loss) and a regulatization loss. We fit the data fidelity term and make sure it follows the expectation
that the number of weights will be as small as possible as the data fidelity term allows. Basically, we derived our 
loss function and equalized it to 0 to obtain our Ridge Regression solution (w* = (Z.T Z + λI)^-1 (Z.T y)). This is
an special kind of linear regression with a regularization term.

The results obtained in both implementations were very similar, as we can see in the in the scores and graphs.
The scores do not deviate more than 5% than scikit-learn's scores. In the graphs, we can see very similar curves. 
We start overfitting and as we increase the value of lambda, we get a better fit until getting a correct performance. 
...
class RidgeRegressionClosedForm(object):
    ...
    def fit(self, z, y, alpha=0.0):
        ...
        z_transpose_z = np.matmul(Z.T, Z)
        add_alpha = z_transpose_z + (alpha * np.identity(len(z_transpose_z)))
        inv = np.linalg.inv(add_alpha)
        self.__weights = np.matmul(np.matmul(inv, Z.T), y)

        # TODO: Compute loss
        zw_minus_y = np.matmul(Z, self.__weights) - y

        loss_data_fidelity = np.matmul(zw_minus_y.T, zw_minus_y) / Z.shape[0]
        loss_regularization = alpha * (np.matmul(self.__weights.T, self.__weights)) / Z.shape[0]
...
if __name__ == '__main__':
    ...
    x_train = np.transpose(x_poly_train, axes=(1,0))
    x_val = np.transpose(x_poly_val, axes=(1,0))
    x_test = np.transpose(x_poly_test, axes=(1,0))
    ...
    for alpha in alphas:
        ...
        score_mse_ridge_ours_train = model_ridge_ours.score(x_train, y_train, 'mean_squared_error')
        print('Training set mean squared error: {:.4f}'.format(score_mse_ridge_ours_train))

        score_r2_ridge_ours_train = model_ridge_ours.score(x_train, y_train)
        ...
        score_mse_ridge_ours_val = model_ridge_ours.score(x_val, y_val, 'mean_squared_error')
        print('Validation set mean squared error: {:.4f}'.format(score_mse_ridge_ours_val))

        score_r2_ridge_ours_val = model_ridge_ours.score(x_val, y_val)
        ...
        score_mse_ridge_ours_test = model_ridge_ours.score(x_test, y_test, 'mean_squared_error')
        print('Testing set mean squared error: {:.4f}'.format(score_mse_ridge_ours_test))

        score_r2_ridge_ours_test = model_ridge_ours.score(x_test, y_test)
