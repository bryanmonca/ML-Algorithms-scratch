...
class LinearRegressionClosedForm(object):
    ...
    def __fit_normal_equation(self, X, y):
        ...
        x_transpose_x_inv = np.linalg.inv(np.matmul(X.T, X))
        self.__weights = np.matmul(np.matmul(x_transpose_x_inv, X.T), y)
    ...
    def fit(self, x, y, solver='normal_equation'):
        ...
        # Turn X from (d, N) to X (N, d) by taking its transpose
        X = x.T

        if solver == 'normal_equation':
            return self.__fit_normal_equation(X, y)
        elif solver == 'pseudoinverse':
            return self.__fit_pseudoinverse(X,y)
        else:
            raise ValueError('Encountered unsupported scoring_func: {}'.format(solver))
...
if __name__ == '__main__':
    ...
    #print(np.max(y_train))  # 23 is not that bad compared to 50
    ...
    for solver in solvers:
        ...
        scores_mse_train = model.score(x_train, y_train, 'mean_squared_error')
        print('Training set mean squared error: {:.4f}'.format(scores_mse_train))
        ...
        scores_mse_val = model.score(x_val, y_val, 'mean_squared_error')
        print('Validation set mean squared error: {:.4f}'.format(scores_mse_val))

        scores_r2_val = scores_mse_val = model.score(x_val, y_val)
        print('Validation set r-squared scores: {:.4f}'.format(scores_r2_val))

        # TODO: Test model on testing set using mean squared error and r-squared
        scores_mse_test = model.score(x_test, y_test, 'mean_squared_error')
        print('Testing set mean squared error: {:.4f}'.format(scores_mse_test))
