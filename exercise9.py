...
class PrincipalComponentAnalysis(object):
    ...
    def __covariance_matrix(self, B):
        ...
        C = 1.0 / (B.shape[0] - 1) * np.matmul(B.T, B)
...
if __name__ == '__main__':
    ...
    # TODO: Compute the number of dimensions in the dataset (N, 8, 8) or (N, 64)
    n_dim = np.prod(X_digits.shape[1:]) # will give us 64 # we can always go back to the vectorize form
    ...
    for k in K:
        ...
        Z = pca.project_to_subspace(X_digits)
        ...
        mse_score = skmetrics.mean_squared_error(X_digits, X_hat)
